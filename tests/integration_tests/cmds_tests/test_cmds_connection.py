...
_g_addAttr_kwargs_map = [(key, val) for key, val in _g_addAttr_kwargs_map.iteritems()]  # convert to tuple

strategy_port_type = strategies.sampled_from(_g_addAttr_kwargs_map)
...
def test_connectAttr_missing_dst(cmds):
    ...
    with pytest.raises(RuntimeError) as exception:
        cmds.connectAttr('transform1.src', 'transform1.dst')
    assert exception.match("The destination attribute 'transform1.dst' cannot be found.")
...
def test_connectAttr_incompatible_ports(cmds):
    ...
    with pytest.raises(RuntimeError) as exception:
        cmds.connectAttr('transform1.src', 'transform1.dst')
    assert exception.match("The attribute '|transform1.src' cannot be connected to '|transform1.dst'.")
...
def test_connectionInfo_both_flag_at_once(cmds):
    ...
    with pytest.raises(RuntimeError) as exception:
        cmds.connectionInfo('transform1.src', sourceFromDestination=True, destinationFromSource=True)
...
def test_disconnectAttr_no_connection(cmds):
    ...
    assert exception.match("There is no connection from 'transform1.dst' to 'transform1.src' to disconnect")


# @given(src_data=strategy_port_type, dst_data=strategy_port_type)
@pytest.fixture(params=itertools.permutations((
    EnumAttrTypes.byte,
...
def test_connect_different_types(request, cmds, maya_session, src_data, dst_data):
    ...
    # for hypothesis
    # type_src, port_src_kwargs = src_data
    # type_dst, port_dst_kwargs = dst_data
    ...
    note('add %r port %r' % (type_src.value, port_src_dagpath))
    cmds.addAttr('transform1', longName=port_src_name, **src_kwargs)
    note('add %r port %r' % (type_dst.value, port_dst_dagpath))
    ...
    if factor:
        assert cmds.connectionInfo(port_src_dagpath, destinationFromSource=True) == [u'unitConversion1.input']
        assert cmds.connectionInfo(port_dst_dagpath, sourceFromDestination=True) == u'unitConversion1.output'
        assert cmds.getAttr('unitConversion1.conversionFactor') == factor
    else:
        assert cmds.connectionInfo(port_src_dagpath, destinationFromSource=True) == [port_dst_dagpath]
        assert cmds.connectionInfo(port_dst_dagpath, sourceFromDestination=True) == port_src_dagpath
